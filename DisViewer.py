...
# 讀取Excel檔案
excel_file = "Ikariam島嶼_New_20260323.xlsx"
...
class InteractivePlot:
    ...
    def submit(self, text):
        try:
            ...
            if xliml <= x <= xlimr and yliml <= y <= ylimr:
                ...
                print("座標範圍應該在 (xliml, yliml) 到 (xlimr, ylimr) 之間" if use_chinese else f"Coordinates should be between (xliml, yliml) and (xlimr, ylimr)")
        except ValueError:
    ...
    def draw_triangle(self, pts):
        ...
        for i in range(3):
            ...
            mid_x = (p1[0] + p2[0]) / 2
            mid_y = (p1[1] + p2[1]) / 2
            if not (xliml <= mid_x <= xlimr and yliml <= mid_y <= ylimr):
                continue
